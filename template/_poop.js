...
export async function before(ctx) {
	...
	// add the plugin's name to our context
	ctx.data.plugin = {
		name,
	};
	...
	// set the name to the name of the plugin
...
export async function after(ctx) {
	...
	const pluginDir = path.relative(process.cwd(), dest);
	...
	console.log(`  ${nodePackageManager} run dev`);
}
